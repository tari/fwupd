 ...
 **/
static gboolean
fu_smbios_setup_from_kernel (FuSmbios *self, const gchar *class_path, GError **error)
{
	...
	for (gsize i = 0; i < (sizeof (KERNEL_DMI_FIELDS) / sizeof (*KERNEL_DMI_FIELDS)); i++) {
		const struct kernel_dmi_field *field = &KERNEL_DMI_FIELDS[i];
		g_autofree gchar *path = g_build_filename (class_path, field->name, NULL);
		g_autoptr(GError) local_error = NULL;
		gchar *file_contents;
		gsize file_length;

		if (!g_file_get_contents(path, &file_contents, &file_length, &local_error)) {
			g_debug ("unable to read SMBIOS data from %s: %s", path,
				local_error->message);
			continue;
		}
		/* trim trailing newline added by kernel */
		if (file_contents[file_length - 1] == '\n')
			file_contents[file_length - 1] = 0;

		if (!field->encode (self, file_contents, field->type, field->offset, &local_error)) {
			g_warning ("failed to parse SMBIOS data from %s: %s", path,
				   local_error->message);
			continue;
		}
	...
	if (!any_success) {
		g_set_error (error, FWUPD_ERROR, FWUPD_ERROR_NOT_SUPPORTED,
			     "failed to read any SMBIOS values from %s", class_path);
	}
	return any_success;
}
...
fu_smbios_setup (FuSmbios *self, GError **error)
{
	...
	if (g_file_test (path, G_FILE_TEST_EXISTS)) {
		if (fu_smbios_setup_from_path (self, path, error))
			return TRUE;
		/* try values the kernel parsed if failed to read raw tables; these
		 * are world-readable whereas the raw tables are not */
		if (g_file_test (path_dmi_class, G_FILE_TEST_IS_DIR)) {
			g_autoptr(GError) class_error = NULL;
			g_debug ("trying to read %s: %s", path_dmi_class, (*error)->message);
			if (fu_smbios_setup_from_kernel (self, path_dmi_class, &class_error))
				return TRUE;

			g_prefix_error (error,
					"failed to read SMBIOS from %s (%s), tried after direct access failed: ",
					path_dmi_class, class_error->message);
		}
		return FALSE;
	}
